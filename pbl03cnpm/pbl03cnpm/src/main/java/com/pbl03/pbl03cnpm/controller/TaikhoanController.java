...
public class TaikhoanController {
	...
	List<Taikhoan> getAllAdmin(){
		return taikhoanRepo.findAll().stream()
							.filter(new Predicate<Taikhoan>() {
								@Override
								public boolean test(Taikhoan t) {
									if(t.getCustomer() == null)
										return t.getEmployee().getMaNV().contains("admin");
									else return false;
								}
	...
	List<Taikhoan> getAllEmployee(){
		return taikhoanRepo.findAll().stream()
				.filter(new Predicate<Taikhoan>() {
					...
					public boolean test(Taikhoan t) {
						if(t.getCustomer() == null)
							return t.getEmployee().getMaNV().contains("employ");
						else return false;
					}
