...
CREATE TABLE `hoadon` (
  ...
  `Mota` varchar(50) DEFAULT '" "',
  `TrangThai` tinyint DEFAULT '0',
  PRIMARY KEY (`MaHD`),
  KEY `fk_hd_kh` (`MaKH`),
  KEY `fk_hd_nv` (`MaNV`),
  KEY `fk_hd_dvvc` (`MaDVVC`),
  KEY `fk_hd_tinh` (`MaTinh`),
  CONSTRAINT `fk_hd_dvvc` FOREIGN KEY (`MaDVVC`) REFERENCES `donvivanchuyen` (`MADVVC`),
  CONSTRAINT `fk_hd_kh` FOREIGN KEY (`MaKH`) REFERENCES `khachhang` (`MaKH`),
  CONSTRAINT `fk_hd_nv` FOREIGN KEY (`MaNV`) REFERENCES `nhanvien` (`MaNV`),
  CONSTRAINT `fk_hd_tinh` FOREIGN KEY (`MaTinh`) REFERENCES `tinh` (`MaTinh`)
